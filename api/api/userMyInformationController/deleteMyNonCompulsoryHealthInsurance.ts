...
export async function deleteMyNonCompulsoryHealthInsurance(options: Partial<Parameters<typeof client>[0]> = {}): Promise<ResponseConfig<DeleteMyNonCompulsoryHealthInsuranceMutationResponse>["data"]> {
    const res = await client<DeleteMyNonCompulsoryHealthInsuranceMutationResponse>({ method: "delete", url: `/api/users/me/non-compulsory-health-insurance`, baseURL: "http://localhost:8080", ...options });
    return res.data;
}
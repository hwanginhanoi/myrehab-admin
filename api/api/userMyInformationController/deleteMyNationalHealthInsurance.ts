...
export async function deleteMyNationalHealthInsurance(options: Partial<Parameters<typeof client>[0]> = {}): Promise<ResponseConfig<DeleteMyNationalHealthInsuranceMutationResponse>["data"]> {
    const res = await client<DeleteMyNationalHealthInsuranceMutationResponse>({ method: "delete", url: `/api/users/me/national-health-insurance`, baseURL: "http://localhost:8080", ...options });
    return res.data;
}
...
export async function updateMyNationalHealthInsurance(data?: UpdateMyNationalHealthInsuranceMutationRequest, options: Partial<Parameters<typeof client>[0]> = {}): Promise<ResponseConfig<UpdateMyNationalHealthInsuranceMutationResponse>["data"]> {
    const res = await client<UpdateMyNationalHealthInsuranceMutationResponse, UpdateMyNationalHealthInsuranceMutationRequest>({ method: "put", url: `/api/users/me/national-health-insurance`, baseURL: "http://localhost:8080", data, ...options });
    return res.data;
}
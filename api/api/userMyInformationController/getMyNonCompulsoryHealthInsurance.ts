...
export async function getMyNonCompulsoryHealthInsurance(options: Partial<Parameters<typeof client>[0]> = {}): Promise<ResponseConfig<GetMyNonCompulsoryHealthInsuranceQueryResponse>["data"]> {
    const res = await client<GetMyNonCompulsoryHealthInsuranceQueryResponse>({ method: "get", url: `/api/users/api/users/me/non-compulsory-health-insurance`, baseURL: "http://localhost:8080", ...options });
    return res.data;
}
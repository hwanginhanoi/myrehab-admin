...
export function DoctorFeedbackAlert({ doctorNotes, reviewedAt, doctorName }: DoctorFeedbackAlertProps) {
  return (
    <Alert variant='destructive' className='border-red-500'>
      <AlertTriangle className='h-4 w-4' />
      <AlertTitle>Yêu cầu bị từ chối</AlertTitle>
      <AlertDescription className='mt-2 space-y-1'>
        <p className='font-medium'>Phản hồi từ bác sĩ{doctorName ? ` (${doctorName})` : ''}:</p>

...
export function ReviewActionPanel({ requestId }: ReviewActionPanelProps) {
  ...
  const approveMutation = useApproveCourseRequest({
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: [{ url: '/api/doctors/course-requests' }] })
        toast.success('Đã phê duyệt yêu cầu thành công')
        navigate({ to: '/doctor-course-requests', search: { page: 1, pageSize: 10 } })
      },
    },
  })
  ...
  const handleConfirmApprove = () => {
    approveMutation.mutate({
      id: requestId,
      data: doctorNotes.trim() ? { doctorNotes: doctorNotes.trim() } : undefined,
    })
        ...
        <CardContent className='space-y-4'>
          <div className='space-y-2'>
            <Label htmlFor='doctorNotes'>
              Ghi chú của bác sĩ{' '}
              <span className='text-muted-foreground text-xs'>(bắt buộc khi từ chối)</span>
            </Label>
            <Textarea
              id='doctorNotes'
      ...
      <AlertDialog open={showApproveDialog} onOpenChange={setShowApproveDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bạn có chắc chắn muốn phê duyệt yêu cầu này? Khóa tập sẽ được tạo và gán cho bệnh nhân.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={showRejectDialog} onOpenChange={setShowRejectDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bạn có chắc chắn muốn từ chối yêu cầu này? Kỹ thuật viên sẽ nhận được phản hồi và có thể chỉnh sửa lại.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Hủy</AlertDialogCancel>
            <AlertDialogAction onClick={handleConfirmReject} className='bg-destructive hover:bg-destructive/90'>
              Từ chối
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>

...
export function CourseAssignmentsTableToolbar({
  table,
}: CourseAssignmentsTableToolbarProps) {
  ...
  const [dateRange, setDateRange] = useState<DateRange | undefined>(() => {
    const startDateFilter = table.getColumn('startDate')?.getFilterValue() as
      | string
      | undefined
    const endDateFilter = table.getColumn('endDate')?.getFilterValue() as
      | string
      | undefined
  ...
  const handleDateRangeChange = (range: DateRange | undefined) => {
    ...
    if (range?.from) {
      table
        .getColumn('startDate')
        ?.setFilterValue(format(range.from, "yyyy-MM-dd'T'00:00:00"))
    } else {
    ...
    if (range?.to) {
      table
        .getColumn('endDate')
        ?.setFilterValue(format(range.to, "yyyy-MM-dd'T'23:59:59"))
    } else {

...
import { useCreateCourseRequest, useUpdateCourseRequest, type CourseAssignmentRequestResponse } from '@/api'
import type { DayWithExercises, CustomExercise } from '@/features/courses/components/course-assignment-screen'
...
function daysReducer(state: DaysState, action: AssignmentAction): DaysState {
  switch (action.type) {
    ...
    case 'DELETE_DAY': {
      ...
      let newDayNumber = 1
      for (const [dayNum, day] of Array.from(state.entries()).sort(([a], [b]) => a - b)) {
    ...
    case 'ADD_EXERCISE_TO_DAY': {
      ...
      newDays.set(dayNumber, { ...day, exercises: [...day.exercises, newExercise] })
      return newDays
    }
    ...
    case 'MOVE_EXERCISE': {
      ...
      if (fromDay !== toDay) {
        const exists = targetDay.exercises.some(
          (ex) => ex.exerciseId === sourceDay.exercises[exerciseIndex].exerciseId
        )
        if (exists) {
      ...
      sourceDay.exercises.forEach((ex, idx) => { ex.orderInDay = idx + 1 })
      targetDay.exercises.forEach((ex, idx) => { ex.orderInDay = idx + 1 })
    ...
    case 'REMOVE_EXERCISE': {
      ...
      day.exercises.forEach((ex, idx) => { ex.orderInDay = idx + 1 })
    ...
    case 'REORDER_EXERCISES': {
      ...
      const day = newDays.get(dayNumber)
      if (!day) return state
      const reordered = exercises.map((ex, idx) => ({ ...ex, orderInDay: idx + 1 }))
...
export function CourseRequestForm({
  ...
  // Initialize days state from existing request or empty
  const initDays = (): DaysState => {
    if (!existingRequest?.courseDetails) {
      const m = new Map<number, DayWithExercises>()
      m.set(1, { dayNumber: 1, exercises: [] })
      return m
    }
    const details = existingRequest.courseDetails as unknown as ParsedCourseDetails
      ...
      for (const day of details.days) {
        m.set(day.dayNumber, {
          ...
          exercises: (day.exercises ?? []).map((ex, idx: number) => ({
            ...
            exerciseTitle: `Bài tập #${ex.exerciseId}`,
            orderInDay: idx + 1,
  ...
  const [customizedDays, dispatch] = useReducer(daysReducer, undefined, initDays)

  const details = existingRequest?.courseDetails as unknown as ParsedCourseDetails | undefined
  ...
  const createMutation = useCreateCourseRequest({
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: [{ url: '/api/trainer/course-requests' }] })
  ...
  const updateMutation = useUpdateCourseRequest({
    mutation: {
      onSuccess: () => {
        ...
        navigate({ to: '/course-requests/$id', params: { id: String(requestId) } })
      },
    },
  })
      ...
      <form onSubmit={handleSubmit} className='space-y-6'>
          ...
          <CardContent className='space-y-4'>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder='Nhập tên khóa tập...' maxLength={200} {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <div className='space-y-2'>
              ...
              <p className='text-sm text-muted-foreground'>
                {customizedDays.size} ngày (được cập nhật tự động theo số ngày bên dưới)
              </p>
            </div>
        ...
        <div className='flex justify-end gap-2'>
          ...
          <Button type='submit' disabled={isPending}>
            {isPending ? 'Đang lưu...' : isEditMode ? 'Cập nhật & Nộp lại' : 'Gửi yêu cầu'}
          </Button>
        </div>
      </form>
    </Form>

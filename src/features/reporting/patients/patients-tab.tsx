...
export function PatientsTab() {
  ...
  const summary = useMemo(() => {
    ...
    const inProgress = courses.reduce((s, c) => s + (c.inProgress ?? 0), 0)
    const completed = courses.reduce((s, c) => s + (c.completed ?? 0), 0)
    const rates = courses
  ...
  const packageSummary = useMemo(() => {
    return {
      ...
      activeSubscriptions: packages.reduce(
        (s, p) => s + (p.activeSubscriptions ?? 0),
        0
      ),
      expiredSubscriptions: packages.reduce(
        (s, p) => s + (p.expiredSubscriptions ?? 0),
        0
      ),
    }
  }, [packages])

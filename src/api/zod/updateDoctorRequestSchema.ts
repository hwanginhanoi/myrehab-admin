/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import { z } from 'zod/v4'

/**
 * @description Update request for Doctor staff
 */
export const updateDoctorRequestSchema = z
  .object({
    specialization: z.optional(
      z.string().describe("Doctor's medical specialization")
    ),
    isPublic: z.optional(
      z.boolean().describe('Whether this doctor is visible in the mobile app')
    ),
    profileImageUrl: z.optional(
      z.string().describe("Object key or URL of the doctor's profile image")
    ),
    email: z.email().min(1).describe('Staff email address'),
    fullName: z.string().min(1).describe('Staff full name'),
    staffType: z
      .enum(['DOCTOR', 'TRAINER', 'ADMIN', 'SUPER_ADMIN'])
      .describe('Staff type discriminator'),
  })
  .describe('Update request for Doctor staff')

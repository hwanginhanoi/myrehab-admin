...
import { updateCategory } from '@/api/api/exerciseCategoryManagementController/updateCategory';
import { ExerciseCategoryResponse } from '@/api/types/ExerciseCategoryResponse';
import { UpdateExerciseCategoryRequest } from '@/api/types/UpdateExerciseCategoryRequest';
...
export default function EditExerciseCategoryPage() {
  ...
  const onSubmit = async (data: ExerciseCategoryFormData) => {
    ...
    try {
      const requestData: UpdateExerciseCategoryRequest = {
        name: data.name.trim(),
        description: data.description.trim() || undefined,
        type: data.type as UpdateExerciseCategoryRequest['type'],
      };

      await updateCategory(Number(categoryId), requestData);

      toast.success('Cập nhật danh mục bài tập thành công');
      router.push(`/dashboard/exercise-categories/${categoryId}`);
    } catch (error) {
      console.error('Error updating exercise category:', error);
      toast.error('Có lỗi xảy ra khi cập nhật danh mục bài tập');
    } finally {
      ...
      <div className="m-9 mt-0 mb-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-4xl font-bold text-[#EF7F26] mb-2">Chỉnh sửa danh mục bài tập</h1>

...
import { getMyCustomCourses } from '@/api/api/courseManagementController/getMyCustomCourses';
...
export default function CustomCoursesPage() {
  ...
  const fetchCourses = useCallback(async () => {
    try {
      ...
      const data = await getMyCustomCourses({
        pageable: {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to fetch custom courses';
      setError(errorMessage);
      toast.error('Failed to load custom courses', {
    ...
    <div className="flex flex-1 flex-col">
      ...
      <div className="m-9 mt-9 mb-6">
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                <h1 className="text-4xl font-bold text-[#EF7F26] mb-2">Khóa học tùy chỉnh</h1>
                <p className="text-base text-[#71717A]">Quản lý các khóa học tùy chỉnh của bạn</p>
              </div>
              <Button
                className="bg-[#6DBAD6] text-white px-4 py-2 rounded-md flex items-center gap-2 shadow-sm hover:bg-[#6DBAD6]/90"
                onClick={() => router.push('/dashboard/custom-courses/create')}
              >
                <Plus className="w-5 h-5" />
                Khóa học tùy chỉnh mới
              </Button>
            </div>
                    ...
                    {table.getRowModel().rows?.length ? (
                              ...
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem
                                    onClick={() => {
                                      if (course.id) {
                                        router.push(`/dashboard/custom-courses/${course.id}`);
                                      }
                                    }}
                                  >
                                  ...
                                  <DropdownMenuItem
                                    onClick={() => {
                                      if (course.id) {
                                        router.push(`/dashboard/custom-courses/${course.id}/edit`);
                                      }
                                    }}
                                  >
                      ...
                      <TableRow>
                        <TableCell colSpan={7} className="h-24 text-center">
                          Không có khóa học tùy chỉnh nào.
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
